...
- Draw $\tau_1$,...,$\tau_d$ such that $\mathbb{P}(\tau_i\ge t)=\exp(-\int_0^t \lambda_i(\xi^{(k-1)}+\theta^{(k-1)}s,\theta^{(k-1)})ds)$
- Let $i_0=\operatorname{argmin}(\tau_1,...,\tau_d)$ and $\tau=\tau_{i_0}$
- Set $\xi^{(k)}=\xi^{(k-1)}+\tau\theta^{(k-1)}$ and $\theta^{(k)}_i=\theta^{(k-1)}_i$ for $i\neq i_0$, $\theta^{(k)}_{i_0}=-\theta^{(k-1)}_{i_0}$  
...
* calculate $\int_0^t M(s) \ ds$, which is quadratic or linear
* find $\textit{explicitly}$ the generalised inverse $G(y) = \inf \{ t \geq 0: \int_0^t M(s) \ ds \geq y \}$ 
* sample $\tau = G( - \log U)$ for $U \sim U[0,1]$.  
...
* sample $\tilde \tau_i$ as clocks with rates $M_i$
* find $i_0 = \arg \min_i \tilde \tau_i$
* accept $\tilde \tau_{i_0}$ as the time until the next flip with probability $m_{i_0} (\tilde \tau_{i_0})/M_{i_0} (\tilde \tau_{i_0})$. 
...
#### Globally uniformly Lipschitz-continuous
